...
void initAllegro();
void updateDisplay(int currentDisplay);
...
int32 CVICALLBACK EveryNCallback(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void *callbackData)
{
	...
	if( read>0 ) {
		...
		row = (int) 512-disp*scale;
		printf("Acquired %d samples. Current average is %f V. Total %d.\r",read,disp,totalRead+=read);
		updateDisplay(disp);
	}
